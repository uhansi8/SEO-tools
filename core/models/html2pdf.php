    ...
    class HTML2PDF
    {
        ...
        public function Output($name = '', $dest = false)
        {
            ...
            if ($this->_debugActif) {
                ...
                $this->pdf->Close();
                $time_elapsed_secs = round(microtime(true) - SATIME, 3);
                echo '<br> Total Time Taken: '.$time_elapsed_secs.' Sec<br>';
                exit;
            }
...
'.$content.'
        ...
        protected function _tag_open_A($param)
        {
            ...
            if (preg_match('/^#([^#]+)$/isU', $this->_isInLink, $match)) {
                //Added by B-A-L-A-J-I  (P-r-o-T-h-e-m-e-s.B-I-Z)
                //Date: 4/5/2016
                $name = $match[1];
                if(check_str_contains($match[1],'*')){
                    $pageNum = (int)substr($match[1],1);
                    if (!isset($this->_lstAnchor[$name])) $this->_lstAnchor[$name] = array($this->pdf->AddLink(), false);
                    $this->_isInLink = $this->_lstAnchor[$name][0];
                    $this->pdf->SetLink($this->_lstAnchor[$name][0], 0, $pageNum);
                }else{
                    if (!isset($this->_lstAnchor[$name])) $this->_lstAnchor[$name] = array($this->pdf->AddLink(), false);
                    $this->_isInLink = $this->_lstAnchor[$name][0];
                }
