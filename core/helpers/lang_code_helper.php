...
function removeLang($code,$con){
    ...
    mysqli_query($con,$query);
    $query1 = "ALTER TABLE lang DROP COLUMN lang_$code";
    mysqli_query($con,$query1);
    return true;
}
...
function langStatusChange($code, $status, $con){
    ...
    foreach($allLangs as $la){
        if(in_array($code,$la, true)){
            $newLangs[] = array($la[0],$la[1],$la[2],$la[3],$la[4],$status,$la[6],$la[7]);
        }else
            $newLangs[] = $la;
    }
    $newLangs = serialize($newLangs);
...
function langUpdateAll($code, $langArr, $con){
    ...
    foreach($allLangs as $la){
        if(in_array($code,$la, true))
            $newLangs[] = array($langArr[0],$la[1],$langArr[1],$langArr[2],$la[4],$langArr[3],$langArr[4],$langArr[5]);
        else
            $newLangs[] = $la;
    }
    $newLangs = serialize($newLangs);
    ...
    if($code != $langArr[1])
        mysqli_query($con,"ALTER TABLE `lang` CHANGE `lang_$code` `lang_".$langArr[1]."` text");
...
function getAvailableLanguages($con){
     ...
     $query = mysqli_query($con, "SELECT available_languages FROM interface WHERE id=1");
...
function getAvailableLanguageCodes($con, $noDefault=false){
     $availableLanguages = array();
     $query = mysqli_query($con, "SELECT available_languages FROM interface WHERE id=1");
     $data = mysqli_fetch_assoc($query);
     $langArr = unserialize($data['available_languages']); 
     foreach($langArr as $la){
         if($la[5]) {
             if ($noDefault) {
                 if ($la[2] !== DEFAULT_LANG)
                     $availableLanguages[] = $la[2];
             } else {
                 $availableLanguages[] = $la[2];
             }
         }
...
function getLangData($langCode,$con){
    if(strlen($langCode) == 2){
        ...
        if($langOkay){
            $langCodeData = mysqli_query($con, "SELECT code, lang_$langCode FROM lang");
    ...
    $_SESSION[N_APP.'UserSelectedLang'] = getLang($con); 
...
function customLangLink($link='', $langCode, $subPath, $return=false){
    $langShortCode = '';
    if(!defined('BASEURL'))
        die('Base URL not set!');
    if(defined('LANG_SHORT_CODE'))
        $link = str_replace('/'.LANG_SHORT_CODE, '' , $link);
    $link = str_replace($subPath, '' , $link); 
    if($return)
        return BASEURL.$langCode.$link;
    else
        echo BASEURL.$langCode.$link;
}