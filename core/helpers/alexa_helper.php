...
function alexaRank2021($site){
    ...
    $outData = curlGET("https://www.alexa.com/siteinfo/$site");
    ...
    $cloop = 1;
    $top_countryData_rw = $top_countryData = $countryNameArr = $keywordsData = array();
    ...
    $keywordsJsonData =  getCenterText('<script type="application/json" id="topKeywordsJSON">', '</script>', $outData);
    $keywordsArr = json_decode($keywordsJsonData, true);

    if(isset($keywordsArr[$site]))
        $keywordsData = $keywordsArr[$site];

    return array('rank' => $alexa_rank, 'popularity' => $alexa_pop, 'regional' => $regional_rank, 'backlinks' => $alexa_back, 'top_country' => $top_countryData, 'keywords' => $keywordsData);
}
