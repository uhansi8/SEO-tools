...
//Premium Image Verfication Check
if(isset($_SESSION[N_APP.'premiumToken'])){
    $capCheck = false;
    $capCheck = $_SESSION[N_APP.'premiumToken'][9];
    if($capCheck)
        $toolCap = false;
}
...
elseif($toolUid == 'PR10') {
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            $myHost = ucfirst($host);
            $alexa = alexaRank2021($host);
            $alexa_rank = $alexa['rank'];
            $alexa_pop = $alexa['popularity'];
            $regional_rank = $alexa['regional'];
            $alexa_back = $alexa['backlinks'];
            $alexa_top_country = $alexa['top_country'];
            $alexa_keywords = $alexa['keywords'];
            }
        }
    }
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            if (filter_var($my_url, FILTER_VALIDATE_URL) === false) {
                ...
                $regUserInput = $my_url;
                $pageSpeedInsight = pageSpeedInsightTool($my_url, 'desktop', true);
            }
        }
    }
}
